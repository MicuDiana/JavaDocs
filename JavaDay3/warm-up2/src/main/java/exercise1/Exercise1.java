...
public class Exercise1 {
    ...
    public List<String> iteratingOverEntries(){
        ...
        for(HashMap.Entry<String, String> entry : countries.entrySet()){
            if(entry.getKey().contains("Q") || entry.getValue().contains("Q")
        || entry.getKey().contains("q") || entry.getValue().contains("q")){
    ...
    public String iteratingOverValues(){
        ...
        for(String capital : countries.values()) {
            if(capital.length() > maxCapital) {
                maxCapital = capital.length();
